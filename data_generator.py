...
# Optional fallback for cloud deployments if local file is missing
# Replace only if you really want a GitHub fallback
GITHUB_URL = (
    "https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/"
    "data/PitWall_Analytics_Cleaned.xlsx"
)

_HEADER_ROW = 2   # real headers are on Excel row 3
...
def _open_excel() -> dict[str, pd.DataFrame]:
    ...
    if "YOUR_USERNAME" not in GITHUB_URL and "YOUR_REPO" not in GITHUB_URL:
        ...
        with urllib.request.urlopen(GITHUB_URL) as resp:
            return pd.read_excel(io.BytesIO(resp.read()), sheet_name=None, header=_HEADER_ROW)
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if missing:
        raise ValueError(f"Missing expected sheet(s): {missing}. Found: {list(xl.keys())}")
...
def _clean_sessions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # safer boolean handling in case Excel stores TRUE/FALSE, Yes/No, 1/0
    df["Is Weekend"] = (
        df["Is Weekend"]
        .astype(str)
        .str.strip()
